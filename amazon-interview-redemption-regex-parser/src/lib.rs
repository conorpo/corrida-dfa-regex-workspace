...
use gerber::*;
use std::{cell::{Cell, OnceCell, Ref, RefCell, UnsafeCell}, collections::HashMap, rc::Rc, str::Chars};
...
fn parse_group<'a,const is_outermost: bool>(mut iter: Chars<'a>, starting_node: NodeAlias<'a>, nfa: &'a Nfa<char>) -> (Chars<'a>, NodeAlias<'a>, NodeAlias<'a>) {
    
    let mut union_nodes = None;
    // Keeps track of the beginning of a concat expression, ended (and needed) on | or )
    let mut concat_start = starting_node;
