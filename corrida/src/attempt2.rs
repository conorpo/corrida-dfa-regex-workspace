...
impl<T> Fighter for IsolatedFighter<T> {}
impl<T> From<T> for IsolatedFighter<T> {
    fn from(ele: T) -> Self {
        Self {
            ele,
        }
...
impl<T> Fighter for ConnectedFighter<T> {}
impl<T> From<T> for ConnectedFighter<T> {
    fn from(ele: T) -> Self {
        Self {
            ele,
            edges: Vec::new()
...
impl<T> Fighter for DirectedFighter<T> {}
impl<T> From<T> for DirectedFighter<T> {
    fn from(ele: T) -> Self {
        Self {
            ele,
            outgoing: Vec::new(),
...
pub trait ArenaTrait<F> {
    fn alloc(&self, fighter: F) -> &mut F;
}
...
impl<F> Arena<F> {
    ...
    pub fn alloc_core(&self) -> NonNull<F> {
...
impl<T> Arena<IsolatedFighter<T>> {
    /// Allocate one isolated fighter, holding data T.
    pub fn alloc(&self, fighter: IsolatedFighter<T>) -> &mut IsolatedFighter<T>{
        unsafe { 
...
// Provide macro for these fighters
impl<T> Arena<ConnectedFighter<T>> {
    pub fn alloc(&self, ele: T, fighter: Vec<&ConnectedFighter<T>>) -> &mut ConnectedFighter<T> {
